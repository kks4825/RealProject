			...
			<div class="sub_cont">
				...
				<div id='module_board' class='him_module' rel='qna'><div class="sub_board">
					...
					<div class="btn_center">
						<a href="#" onclick="location.href='boardNoticeQna.do?pg=${pg}'" class="btn blue">목록</a>
						<c:if test="${sessionScope.memName==boardDTO.name or sessionScope.memName=='어드민'}">
							<a href="boardModifyQnaForm.do?seq=${boardDTO.seq}&pg=${pg}" class="btn gray">수정</a>
							<a href="boardDeleteQna.do?seq=${boardDTO.seq}" class="btn gray">삭제</a>
						</c:if>
						<a href="boardReplyQnaForm.do?seq=${boardDTO.seq }&answer=${boardDTO.id }&pseq=${boardDTO.seq }&pg=${pg }&category=${category}" class="btn gray">답글</a>
					</div><!--btn_center-->
					</div><!--sub_board-->
				</div>				
			</div>
