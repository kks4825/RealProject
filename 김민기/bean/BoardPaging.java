...
public class BoardPaging {
	...
	public void makePagingHTML(){
		...
		if(startPage>pageBlock)
			pagingHTML.append("<span id=paging onclick=boardPaging("+(startPage-1)+")><img src='image/board/page_left.png' style='height:30px; width:30px;'>&nbsp;&nbsp;</span>");
		...
		if(endPage<totalP)
			pagingHTML.append("&nbsp;&nbsp;<span id=paging onclick=boardPaging("+(endPage+1)+")><img src='image/board/page_right.png' style='height:30px; width:30px;'></span>");	
	}
	
	public void makePagingHTML(Map<String,String> map){
		pagingHTML = new StringBuffer();
		
		int totalA = boardDAO.getTotalA(map);	//검색 된 총글수
		int totalP = (totalA+pageSize-1)/pageSize;	//총페이지수
